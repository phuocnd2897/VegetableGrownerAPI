...
namespace VG.Service.Service
{
    ...
    public class VegetableService : IVegetableService
    {
        ...
        public VegetableService(IVegetableRepository vegetableRepository, IVegetableDescriptionRepository vegetableDescriptionRepository,
            IVegetableCompositionRepository vegetableCompositionRepository, IVegetableImageService vegetableImageService, IPostRepository postRepository,
            ...
            else
            {
                if (newItem.NameSearch != "" && newItem.NameSearch != null)
                {
                    ...
                    if (ListVegName.Count() > 0)
                    {
                        ...
                        if (this._labelRepository.GetMulti(s => s.LabelName == newItem.NameSearch).Count() <= 0)
                        {
        ...
        {
            var result = this._vegetableRepository.GetSingle(s => s.Id == Id, new string[] { "Posts" });
            ...
            {
                var composition = this._vegetableCompositionRepository.GetMulti(s => s.VegetableDescriptionId == des.Id);
        ...
        public IEnumerable<VegetableResponseModel> SearchByName(string searchValue)
        {
            ...
            foreach (var label in Labels)
            {
                ...
                {
                    var vegDetail = this._vegetableDescriptionRepository.GetSingle(s => s.Id == label.VegetableComposition.VegetableDescriptionId && s.Status == true);
                    if (vegDetail != null)
                    {
        ...
        public VegetableRequestModel Update(VegetableRequestModel newItem, string phoneNumber, string savePath, string url)
        {
            ...
            if (detail.Where(s => s.VegetableCompositionId == "1").FirstOrDefault().AccountId != null)
            {
                ...
                if (newItem.NewImages != null)
                {
                    var idImg = detail.Where(s => s.VegetableCompositionId == "4").FirstOrDefault().Id;
        ...
        public IEnumerable<VegetableResponseModel> CheckVegetableInGarden(string Id, string Name, string PhoneNumber)
        {
            ...
            var yourVeg = this._vegetableRepository.GetMulti(s => s.VegetableDescriptionId == Id && account.Gardens.Select(q => q.Id).Contains(s.Garden.Id), new string[] { "Garden" }).ToList();
        ...
        public void IsAccept(string Id, int status)
        {
            ...
            if (status == 2)
            {
                ...
                feat.Status = true;
                img.Status = true;
                var label = this._labelRepository.GetSingle(s => s.StandsFor == "CD" && s.VegetableComposition.VegetableDescriptionId == name.Id, new string[] { "VegetableComposition" });
                label.VegCompositionId = "3";
                this._labelRepository.Update(label);
            ...
            else
            {
                ...
                var compositionLabel = this._vegetableCompositionRepository.GetSingle(s => s.VegetableDescriptionId == name.Id, new string[] { "Labels" });
                var compositionKeyWord = this._vegetableCompositionRepository.GetSingle(s => s.VegetableDescriptionId == feat.Id, new string[] { "Keywords" });
                this._vegetableCompositionRepository.Delete(compositionLabel);
                this._vegetableCompositionRepository.Delete(compositionKeyWord);
        ...
        public IEnumerable<VegetableResponseModel> SearchVegetableSharedByDescription(string searchValue)
        {
            ...
            List<VegetableResponseModel> vegetableResponseModels = new List<VegetableResponseModel>();
            var listVegetableShared = this._postRepository.GetMulti(s => s.Quantity > 0).Select(s => s.VegetableId).Distinct().ToList();
